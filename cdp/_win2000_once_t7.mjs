...
try {
  ...
  const VALUES = ['Leo Xm', 'oncetools.zak@92ng.com', 'Zakaihu', 'https://zakaihu.com/', 'Free general web directory with editor reviewed, quality listings.', 'Zakaihu is a free general web directory and resource guide. Editors review each submission to keep listings relevant and useful for visitors browsing business, technology, home and lifestyle categories.', 'Free'];
  for (let i = 0; i < VALUES.length; i++) {
    const f = await focusField(i);
    console.log('FOCUS' + i + ':', f);
    if (String(f).startsWith('OK')) { await typeFocused(VALUES[i]); }
  }
  await cdp.send('Page.captureScreenshot').then(r => fs.writeFileSync('D:/Github/backlink_skills/_win1000_once_v2_filled.png', Buffer.from(r.data, 'base64')));
  ...
  await cdp.send('Page.captureScreenshot').then(r => fs.writeFileSync('D:/Github/backlink_skills/_win1000_once_v2_after.png', Buffer.from(r.data, 'base64')));
