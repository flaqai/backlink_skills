...
for (const [x,y] of [[615,496],[615,592],[712,496]]) await click(x,y);
await click(794,679);
await sleep(3000);
...
if (tk.indexOf('HAS') >= 0) {
  await ev('mouseMoved',{x:690,y:490}); await sleep(150);
  await ev('mousePressed',{x:690,y:490,button:'left',clickCount:1}); await sleep(100);
  await ev('mouseReleased',{x:690,y:490,button:'left',clickCount:1});
  await sleep(6000);
  console.log('URL:', await c.evalT('location.href', 6000));
  console.log('BODY:', await c.evalT("document.body.innerText.split(String.fromCharCode(10)).filter(function(s){return s.trim();}).slice(0,12).join(' | ')", 7000));
  ...
  console.log('JOIN shot');
} else {
  const s = await c.send('Page.captureScreenshot', {format:'jpeg', quality:70}).catch(()=>null);
  if(s) writeFileSync('D:/Github/seoadminC/storage/_reg0000/inube_f9.jpg', Buffer.from(s.data,'base64'));
  console.log('LAST shot9');
}
