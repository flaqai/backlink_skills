...
const shot = async n => { const s = await c.send('Page.captureScreenshot', {format:'png'}).catch(()=>null); if(s) writeFileSync(`D:/Github/seoadminC/storage/_reg0000/${n}.png`, Buffer.from(s.data,'base64')); };
// 重新点 checkbox
await click(437,417);
await sleep(4000);
await shot('inube_r2_c1');
console.log('challenge open, shot taken');
