...
let tab = tabs.find((t) => t.type === 'page' && t.url.includes('bedirectory.com/submit'));
if (CMD === 'fill') {
...
if (CMD === 'fill') {
  await cmd('Page.navigate', { url: 'https://www.bedirectory.com/submit.php' });
  await sleep(4000);
  const fill = await evalJs(`(() => {
    ...
    let r = [];
    r.push(set('input[name="TITLE"]', 'Generator for House'));
    r.push(set('input[name="URL"]', 'https://generatorforhouse.org'));
    r.push(set('textarea[name="DESCRIPTION"]', 'Generator for House is a free home backup power resource with wattage calculators, fuel type comparisons and installation checklists that help homeowners pick the right generator for outages.'));
    r.push(set('input[name="OWNER_NAME"]', 'Leo Xm'));
    r.push(set('input[name="OWNER_EMAIL"]', 'dir.bedirectory@92ng.com'));
  ...
  await shot('D:/Github/seoadminC/storage/tmp/_w1000_bed_page.png');
  process.exit(0);
}
...
if (CMD === 'shotcap') {
  ...
  const g = JSON.parse(geo);
  await shot('D:/Github/seoadminC/storage/tmp/_w1000_bed_cap.png', { x: Math.max(0, g.x), y: Math.max(0, g.y), width: g.w, height: g.h, scale: 1 });
