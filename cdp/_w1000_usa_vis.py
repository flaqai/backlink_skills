...
if not m:
    ...
    open("_w1000_usa_s2.html", "w", encoding="utf-8").write(h2)
    sys.exit(1)
imagehash = m.group(1)
...
if not CAPTCHA_CODE:
    ...
    if img_url:
        ...
        open("_w1000_usa_captcha.png", "wb").write(data)
...
payload = {
    ...
    "DESCRIPTION": DESC,
    "OWNER_NAME": "Leo Xm",
    "OWNER_EMAIL": "dir.usawebsitesdirectory@92ng.com",
    "IMAGEHASH": imagehash,
...
if not ok:
    for kw in ["error", "not valid", "already", "incorrect", "required"]:
        if kw in low:
            ...
            break
open("_w1000_usa_last.html", "w", encoding="utf-8").write(h3)
