...
try {
  ...
  const VALUES = ['Leo Xm', 'once.qr@92ng.com', 'QR Code Generator VIP', 'https://crazyshark.org/', 'Free online QR code generator with custom colors and logos.', 'QR Code Generator VIP is a free online QR code generator. Create custom QR codes with colors, logos and frames, download high-resolution PNG or SVG files, no signup required.', 'Free'];
  for (let i = 0; i < VALUES.length; i++) {
    const f = await focusField(i);
    console.log('FOCUS' + i + ':', f);
    if (String(f).startsWith('OK')) { await typeFocused(VALUES[i]); }
  }
  await cdp.send('Page.captureScreenshot').then(r => fs.writeFileSync('D:/Github/backlink_skills/_win1000_once_v5_filled.png', Buffer.from(r.data, 'base64')));
  ...
  await cdp.send('Page.captureScreenshot').then(r => fs.writeFileSync('D:/Github/backlink_skills/_win1000_once_v5_after.png', Buffer.from(r.data, 'base64')));
