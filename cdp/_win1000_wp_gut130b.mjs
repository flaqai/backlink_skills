...
const st = await c.eval(`(() => {
  const CE = 'iframe,body,[contenteditable]';
  ...
  while ((node = walker.nextNode())) {
    ...
    if (i >= 0) {
      ...
      document.execCommand('createLink', false, 'https://smogcheck-nearme.com');
      const a = [...document.querySelectorAll('a[href="https://smogcheck-nearme.com"]')].pop();
      return JSON.stringify({ linked: !!a, text: a ? a.textContent.slice(0,30) : '', btns: [...document.querySelectorAll('button')].map(x => (x.textContent||'').trim() + (x.disabled ? ':d' : '')).filter(x => /^(update|save|saving|update:d|save:d)/i.test(x)).join('|') });
    }
  }
  return JSON.stringify({ err: 'not-found-in-editable', ceCount: document.querySelectorAll('[contenteditable=true]').length });
})()`);
log('编辑:', String(st).slice(0, 260));
await sleep(1500);
const btn = await c.eval(`(() => {
  const b = [...document.querySelectorAll('button')].filter(x => /^update$/i.test((x.textContent || '').trim()) && !x.disabled).pop();
  if (!b) return 'no-update-btn';
