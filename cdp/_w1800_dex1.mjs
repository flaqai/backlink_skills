...
try {
  ...
  let ready = 'NO';
  for (let i = 0; i < 15; i++) { await sleep(2000); ready = await cdp.eval(`(() => document.querySelector('input[name=de-listing-title]') ? 'YES' : 'NO')()`); if (ready === 'YES') break; }
  if (ready !== 'YES') { console.log('FORM_NOT_READY'); process.exit(1); }
  console.log('FORM_READY');
  const F = [['de-listing-title', TITLE], ['de-listing-website-url', SURL], ['de-listing-description', DESC], ['de-listing-submitter-name', 'Leo Xm'], ['de-listing-submitter-email-address', EMAIL]];
  ...
  const cs = await cdp.eval(`(() => { const s=document.querySelector('select[name=de-listing-category]'); if(!s) return 'NOSEL'; s.value='${CAT}'; s.dispatchEvent(new Event('change',{bubbles:true})); return 'CAT='+s.options[s.selectedIndex].textContent.trim(); })()`);
  console.log(cs);
  const chk = await cdp.eval(`(() => { const g=n=>{const e=document.querySelector('[name='+n+']');return e?e.value.length:0;}; return JSON.stringify({t:g('de-listing-title'),u:g('de-listing-website-url'),d:g('de-listing-description'),e:g('de-listing-submitter-email-address')}); })()`);
