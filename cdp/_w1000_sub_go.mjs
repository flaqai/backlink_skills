...
let tab = tabs.find((t) => t.type === 'page' && t.url.includes('sublimelink.org/submit'));
if (CMD === 'fill') {
...
if (CMD === 'fill') {
  await cmd('Page.navigate', { url: 'https://www.sublimelink.org/submit.php' });
  await sleep(4000);
  const fill = await evalJs(`(() => {
    ...
    r.push(set('input[name="OWNER_EMAIL"]', 'dir.sublimelink@92ng.com'));
  ...
  await shot('D:/Github/seoadminC/storage/tmp/_w1000_sub_page.png');
  process.exit(0);
}
...
if (CMD === 'shotcap') {
  ...
  const g = JSON.parse(geo);
  await shot('D:/Github/seoadminC/storage/tmp/_w1000_sub_cap.png', { x: Math.max(0, g.x), y: Math.max(0, g.y), width: g.w, height: g.h, scale: 1 });
