// _reg0000_daR.mjs — 导航到/posts找Journal入口 (reg0000)
...
await sleep(8000);
const st = await Promise.race([c.evalT(`(() => JSON.stringify({url: location.href.slice(0,100), body: (document.body.innerText||'').replace(/\s+/g,' ').slice(0,400), btns: [...document.querySelectorAll('button,a')].filter(e=>e.offsetParent && /journal|write|create|post/i.test(e.innerText)).map(e=>(e.innerText||'').trim().slice(0,30)).slice(0,6)}))()`, 10000), sleep(11000).then(()=>'TO')]);
...
if (shot && shot !== 'TO') (await import('fs')).writeFileSync('_reg0000_daR.jpg', Buffer.from(shot.data, 'base64'));
ws.close();
