// _reg0000_nuC2.mjs — inube 挑战判点: 格4+格5+验证 (reg0000)
...
for (const [x,y] of [[525,440],[676,440]]) {
  await c.send('Input.dispatchMouseEvent', {type:'mousePressed', x, y, button:'left', clickCount:1});
  await c.send('Input.dispatchMouseEvent', {type:'mouseReleased', x, y, button:'left', clickCount:1});
  await sleep(700);
}
await c.send('Input.dispatchMouseEvent', {type:'mousePressed', x:797, y:679, button:'left', clickCount:1});
await c.send('Input.dispatchMouseEvent', {type:'mouseReleased', x:797, y:679, button:'left', clickCount:1});
await sleep(6000);
...
if (shot && shot !== 'TO') (await import('fs')).writeFileSync('_reg0000_nu_r2.jpg', Buffer.from(shot.data, 'base64'));
ws.close();
