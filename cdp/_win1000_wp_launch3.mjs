  ...
  const r = await c.eval(`(() => {
    for (const x of document.querySelectorAll('button,a')) {
      ...
      if (tt === ${JSON.stringify(exact)} && !x.disabled) {
        ...
        return JSON.stringify({ x: Math.round(r.x + r.width / 2), y: Math.round(r.y + r.height / 2), tag: x.tagName });
      }
    }
    return 'miss';
  })()`);
...
await sleep(12000);
if (await clickText('skip purchase')) {
  // 下一步可能要 continue / launch
  const scan = await c.eval(`(() => {
    const btns = [...document.querySelectorAll('button,a')].map(x => (x.textContent || '').trim()).filter(Boolean);
    return JSON.stringify({ url: location.href.slice(0, 110), btns: btns.slice(0, 22) });
  })()`);
  log('skip后:', String(scan).slice(0, 700));
  if (await clickText('continue')) {}
  const scan2 = await c.eval(`(() => {
    const btns = [...document.querySelectorAll('button,a')].map(x => (x.textContent || '').trim()).filter(Boolean);
    return JSON.stringify({ url: location.href.slice(0, 110), btns: btns.slice(0, 22) });
  })()`);
  log('continue后:', String(scan2).slice(0, 700));
  for (const cand of ['launch site', 'launch your site', 'launch']) { if (await clickText(cand)) break; }
  await sleep(8000);
  const fin = await c.eval(`JSON.stringify({ url: location.href.slice(0, 100) })`);
  log('终态URL:', fin);
}
await sleep(3000);
