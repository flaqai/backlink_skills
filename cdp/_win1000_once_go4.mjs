...
try {
  ...
  const VALUES = ['Leo Xm', 'once.dino@92ng.com', 'Dino Age', 'https://crazyshark.org/', 'Free dinosaur survival browser game with crafting and hunting.', 'Play dinosaur survival games at Dino Age. Gather, craft, hunt, and outlast predators across prehistoric biomes. Free browser game with crafting and survival challenges.', 'Free'];
  for (let i = 0; i < VALUES.length; i++) {
    const f = await focusField(i);
    console.log('FOCUS' + i + ':', f);
    if (String(f).startsWith('OK')) { await typeFocused(VALUES[i]); }
  }
  await cdp.send('Page.captureScreenshot').then(r => fs.writeFileSync('D:/Github/backlink_skills/_win1000_once_v4_filled.png', Buffer.from(r.data, 'base64')));
  ...
  await cdp.send('Page.captureScreenshot').then(r => fs.writeFileSync('D:/Github/backlink_skills/_win1000_once_v4_after.png', Buffer.from(r.data, 'base64')));
