...
let tab = tabs.find((t) => t.type === 'page' && t.url.includes('directoryfire.com/submit'));
if (CMD === 'fill') {
...
if (CMD === 'fill') {
  await cmd('Page.navigate', { url: 'https://www.directoryfire.com/submit.php' });
  await sleep(4000);
  const fill = await evalJs(`(() => {
    ...
    let r = [];
    r.push(set('input[name="TITLE"]', 'Zakaihu'));
    r.push(set('input[name="URL"]', 'https://zakaihu.com'));
    r.push(set('textarea[name="DESCRIPTION"]', 'Zakaihu publishes practical personal finance guides, budgeting templates and investment explainers in plain language, helping readers build savings habits and smarter money decisions.'));
    r.push(set('input[name="OWNER_NAME"]', 'Leo Xm'));
    r.push(set('input[name="OWNER_EMAIL"]', 'dir.directoryfire@92ng.com'));
    const cat = document.querySelector('select[name="CATEGORY_ID"]');
    if (cat) { cat.value = ' Finance '; cat.dispatchEvent(new Event('change', { bubbles: true })); r.push('cat=692'); }
  ...
  await shot('D:/Github/seoadminC/storage/tmp/_w1000_dfire_page.png');
  process.exit(0);
}
...
if (CMD === 'shotcap') {
  ...
  const g = JSON.parse(geo);
  await shot('D:/Github/seoadminC/storage/tmp/_w1000_dfire_cap.png', { x: Math.max(0, g.x), y: Math.max(0, g.y), width: g.w, height: g.h, scale: 1 });
