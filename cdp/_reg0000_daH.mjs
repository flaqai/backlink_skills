// _reg0000_daH.mjs — DA topics三选+Continue (reg0000)
...
for (const name of ['Digital Art','Literature','Tutorials']) {
  const r = await Promise.race([c.evalT(`(() => {
    const els=[...document.querySelectorAll('button,[role=checkbox],label,span')].filter(e=>e.offsetParent && /^${name}$/i.test((e.innerText||'').trim()) && e.getBoundingClientRect().height<70);
    if(!els.length) return 'nf:'+name; const el=els[0]; el.scrollIntoView({block:'center'}); const rc=el.getBoundingClientRect();
...
const st = await Promise.race([c.evalT(`(() => JSON.stringify({url: location.href.slice(0,110), body: (document.body.innerText||'').replace(/\s+/g,' ').slice(0,280), iframes: [...document.querySelectorAll('iframe')].map(f=>f.src.slice(0,70)).slice(0,5)}))()`, 10000), sleep(11000).then(()=>'TO')]);
...
if (shot && shot !== 'TO') (await import('fs')).writeFileSync('_reg0000_da7.jpg', Buffer.from(shot.data, 'base64'));
ws.close();
